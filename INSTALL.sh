...
MISPvars () {
  ...
  GPG_EMAIL_ADDRESS="admin@revlv.local"
...
checkInstaller () {
  ...
  if [[ "${chsum}" == "${INSTsum}" ]]; then
    ...
    for sum in $(echo ${SHA_SUMS} |sed 's/--sha//g'); do
      /usr/bin/wget --no-cache -q -O /tmp/INSTALL.sh.sha${sum} https://raw.githubusercontent.com/emilpancubit/REVLV_Cybersec/main/INSTALL.sh.sha${sum}
...
configMISP () {
  ...
  # delete the user from mysql and log in again using the default admin credentials (admin@revlv.local / admin)
